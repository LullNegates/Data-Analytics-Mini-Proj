...
import q1_analysis
from q1_analysis import _velocity_trend, _power_law_on_improvements
...
class TestVelocityTrend:
    """_velocity_trend returns a VelocityTrendResult DTO."""
    ...
    def test_to_dict_has_required_keys(self):
        ...
        imps   = [50.0, 30.0, 20.0, 10.0, 5.0, 2.0]
        rows   = _make_wr_rows(dates, imps)
        d = _velocity_trend(rows).to_dict()
        assert set(d.keys()) == {"rho", "pvalue", "interpretation"}


# ---------- _power_law_on_improvements ----------

class TestPowerLawOnImprovements:
    """_power_law_on_improvements returns a FitResult DTO or None."""

    def test_returns_none_for_too_few_rows(self):
        rows   = _make_wr_rows(["2020-01-01", "2021-01-01", "2022-01-01"], [10.0, 8.0, 6.0])
        result = _power_law_on_improvements(rows)
        assert result is None

    def test_returns_fitresult_for_sufficient_rows(self):
        dates  = [f"20{10+i}-01-01" for i in range(8)]
        imps   = [100.0, 50.0, 30.0, 20.0, 10.0, 5.0, 3.0, 1.0]
        rows   = _make_wr_rows(dates, imps)
        result = _power_law_on_improvements(rows)
        assert result is not None
        assert result.name in ("power_law", "log")
    ...
    def test_to_dict_has_model_key(self):
        ...
        result = _power_law_on_improvements(rows)
        assert result is not None
        assert "model" in result.to_dict()
    ...
    def test_zero_improvements_excluded(self):
        ...
        rows   = _make_wr_rows(dates, imps)
        result = _power_law_on_improvements(rows)
        assert result is None or result.name in ("power_law", "log")
