...
\finding{3}{Für \textbf{alle 17~Spiele} gelingt eine Kurvenanpassung mit $\mathbf{R^2 > 0{,}70}$ (Median: $0{,}95$). Kein einzelnes Modell dominiert universell: \textit{poly2}~(6/17) erzielt die meisten AIC-Siege, gefolgt von \textit{Gompertz} und exponentiellem Zerfall (je 4/17).}

\finding{4}{\textbf{Alle 17~Spiele} weisen einen statistisch signifikanten Strukturbruch auf (Chow-Test, $p < 0{,}05$). Bei \textbf{8 von 17~Spielen} tritt der Bruchpunkt innerhalb der ersten 10~Weltrekorde auf; bei Spielen mit langer WR-Geschichte liegt er deutlich später (Median: WR~\#16).}
...
\finding{5}{\textbf{5 von 6} auswertbaren Spielen zeigen ein Plateau nach dem Breakthrough ($\phi < 0{,}5$). Nur \textit{Donkey Konga} zeigt eine Beschleunigung ($\phi = 23{,}97$) --- der Breakthrough (Dezember~2024) liegt so kurz zurück, dass die Post-Phase kaum Konsolidierungszeit enthält. Die stärksten Plateaus: \textit{Zelda: OoT} und \textit{The Talos Principle} (je $\phi = 0{,}02$; $\rho = -0{,}79$; $p < 0{,}001$).}

\subsection*{F3b: Annäherung ans theoretische Minimum}

Für 15 von 17~Spielen ist kein stabiler Modell-Floor bestimmbar (\texttt{none\_detected}, $R^2 < 0{,}70$). Für \textit{Half-Life~2} (Floor: $1874{,}7$~s, Gap: $315{,}6$~s) und \textit{Pokémon Red/Blue} (Floor: $6004{,}4$~s, Gap: $215{,}6$~s) konnte ein konvergentes Sättigungsmodell berechnet werden.
...
Tabelle~\ref{tab:q3} zeigt den Kaplan-Meier-Median und die 1-Jahres-Überlebensrate je Genre. Von den 876~Weltrekord-Datenpunkten sind 17~zum Analysezeitpunkt noch aktiv (rechtszensiert).
...
Der Gini-Koeffizient der Verbesserungsgrößen ist in allen Genres hoch ($G = 0{,}655$–$0{,}932$): Ein kleiner Anteil der Weltrekorde macht den Großteil der erzielten Zeitersparnis aus.
...
\finding{9}{Im Jahrzehntvergleich sinkt die arithmetisch mittlere beobachtete Rekordlebensdauer von \textbf{665,8~Tagen} (2000er) auf \textbf{82,4~Tage} (2010er). Der Einbruch fällt zeitlich mit dem starken Wachstum der Speedrunning-Community ab \textbf{2010} zusammen, das den Wettbewerbsdruck erhöhte.}
