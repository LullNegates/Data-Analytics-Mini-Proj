...
Die vollständige Analysepipeline ist durch \textbf{164~automatisierte pytest-Tests} abgedeckt (synthetische In-Memory-Daten, keine externen Dateien). Geprüft werden alle Analysemodule sowie zwei Präzisionsklassen für Ausgaben: \texttt{normalize\_columns} (CSV) und \texttt{normalize\_floats} (JSON). Alle 164~Tests laufen mit 0~Fehlern.
...
Zum Vergleich der Lebensdauerverteilungen zwischen Genres wird der \textbf{Kruskal-Wallis-Test} eingesetzt. Als Effektgröße wird $\eta^2 = (H - k + 1)/(N - k)$ berechnet~\cite{tomczak2014}. Paarweise Unterschiede werden mit dem \textbf{Mann-Whitney-U-Test}~\cite{kim2016} geprüft. Der \textbf{Gini-Koeffizient} misst, wie gleichmäßig die Verbesserungsgrößen verteilt sind ($G = 0$: alle gleich, $G = 1$: eine einzelne Verbesserung dominiert alles).
