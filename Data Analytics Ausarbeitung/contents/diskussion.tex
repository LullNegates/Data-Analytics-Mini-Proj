...
TAS-Daten ermöglichen es, den \emph{Bannister-Effekt}~\cite{wooten2022} direkt zu prüfen: Nicht allein der Nachweis, dass eine Zeit erreichbar ist, sondern die Verbreitung konkreter Techniken treibt Verbesserungen voran. TAS-Routen dokumentieren Strategien Bild für Bild; die Community überträgt diese durch Video-Analyse und Kommentare in die RTA-Praxis. Die Geschwindigkeit, mit der die Lücke zwischen TAS und menschlichem WR schrumpft, zeigt daher auch, wie schnell sich eine neue TAS-Strategie in der Community durchsetzt.

Der $R^2 \geq 0{,}70$-Schwellenwert schließt Spiele ohne stabile Sättigungskurve aus, damit keine unzuverlässigen Schätzungen einfließen. Spiele, die als \enquote{none\_detected} klassifiziert werden (typischerweise junge Spiele wie \textit{Celeste} oder \textit{Hollow Knight}), befinden sich möglicherweise noch in einer aktiven Entdeckungsphase ohne erkennbares theoretisches Minimum.

Bei \textit{Super Mario Bros.} ist die menschliche Community mit einem Gap von \textbf{0,15~s} (99,5~\% der möglichen Reduktion) dem TAS außerordentlich nahe --- ein Ergebnis, das zeigt, wie weit das Spiel bereits ausgereizt ist. \textit{Half-Life~2} und \textit{Super Metroid} haben die dokumentierte TAS-Zeit bereits übertroffen (Gap~= 0~s, 152~\% bzw.\ 150~\% geschlossen), weil RTA-Runner Glitch-Ketten entdeckten, die im analysierten TAS nicht enthalten waren. Das ist ein konkreter Beleg für den Bannister-Effekt: Die TAS-Route wirkte als Existenzbeweis für schnellere Strategien, die Community übernahm und erweiterte sie bis über das TAS-Niveau hinaus.

Der Asymptoten-Proxy liefert für \textit{Half-Life~2} einen Modell-Floor von $1874{,}7$~s --- das sind $2619$~s unterhalb der TAS-Referenzzeit. Dieser Wert erscheint zunächst widersprüchlich, ist aber erklärbar: Der menschliche WR ($2190{,}3$~s) liegt bereits unter dem historischen TAS-Wert ($4494{,}0$~s), das Modell sagt daher folgerichtig eine weitere Verbesserung über diesen veralteten TAS-Stand vorher. Für 15~von 17~Spielen existiert kein konvergentes Sättigungsmodell (\texttt{none\_detected}), da sich die Community noch in einer aktiven Entdeckungsphase befindet.

\placeholder{TAS-Zeitreihenkorrelation interpretieren: Zeigt der Mann-Whitney-U-Test signifikant größere WR-Sprünge nach TAS-Meilensteinen? Falls ja, stützt das die Diffusionshypothese nach Wooten~\cite{wooten2022} direkt. Falls nicht, mögliche Erklärungen diskutieren (zu kleines Zeitfenster, zu wenige TAS-Meilensteine, verzögerte Übernahme).}
